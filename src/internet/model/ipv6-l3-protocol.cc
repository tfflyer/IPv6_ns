 ...
 *
 * Author: S
 * ebastien Vincent <vincent@clarinet.u-strasbg.fr>
 */
...
{
#if 0 //show the trace
	if(m_node->GetId() == 2 || true){

		Ptr<Packet> t_packet = p->Copy ();
	    Ipv6Header t_hdr;
	    t_packet->RemoveHeader (t_hdr);
	    uint8_t t_nextHeader = t_hdr.GetNextHeader ();
	    std::cout << "receive from node:"<< m_node->GetId () <<" protocol:"<< protocol <<"\t"<< from <<"\t"<< to << "\t" << "next header is " << +t_nextHeader << std::endl;
	}
  ...
  uint8_t n1=GetNAddresses(interface);
    uint8_t buf1[16];
    Ipv6Address target1;

    for (uint32_t i = 0; i < n1; i++)
            {
              Ipv6InterfaceAddress test1 = GetAddress (interface, i);
            target1=test1.GetAddress();}
    target1.GetBytes (buf1);
  ...
  hdri=hdr;
    uint8_t ttls;
    ttls=hdri.GetHopLimit();
    Ipv6Address srci;
    srci=hdri.GetSourceAddress();
    uint8_t buf[16];
    srci.GetBytes (buf);
  Ptr<Packet> packets = packet->Copy ();
  UdpHeader header1;
  packets->RemoveHeader(header1);
              ...
              if (addr.IsEqual (hdr.GetDestinationAddress ()))
                {
                  ...
                  if(hdri.GetNextHeader()==176){
                  	  		 if (packets->GetSize () > 0)
                  	  		        {
                  	  		    	  packets->EnablePrinting();
//                  	  		    	std::cout<<"  本接口地址：   ";
//                  	  		    	for(int i=0;i<16;i++){
//                  	  		    	printf("%02X",buf1[i]);
//                  	  		    	}
...
//                  	  		    	std::cout<<"time:"<<time<<std::endl;
                  	  		    	...
                  	  		    	FILE* fout = fopen("ipv6_data.txt","a");
                  	  		    	for(int i=0;i<16;i++)
                  	  		    	{
...
#if 1// start sending trace packet
                  // trace packet : destination port = 9999 && nextHeader = udp(17)
                  ...
                  // std::cout << p ->ToString() << std::endl;
              		  ...
              		  if (+nextHeader == Ipv6Header::IPV6_UDP ){
              			  ...
              			  if(uv->GetValue(1, 100) < 80){
              				  uint8_t buf[16];
              				  addr.GetBytes (buf);
              				  Ptr<Packet> addresspacket = Create<Packet> (buf, 16);

              				  UdpHeader header1;
              				  header1.SetDestinationPort(33);
              				  header1.SetSourcePort(49153);

              				  addresspacket->AddHeader(header1);

              				  Socket::SocketErrno err;
              				  Ptr<NetDevice> oif (0);
              				  Ptr<Ipv6Route> newRoute = 0;

              				  Ipv6Address source = hdr.GetSourceAddress();
              				  Ipv6Address destination = hdr.GetDestinationAddress();

              				  Ipv6Header ipv6header = BuildHeader (source, destination, uint8_t(Ipv6Header::IPV6_UDP), addresspacket->GetSize (), hdr.GetHopLimit(), hdr.GetTrafficClass());
              				  ...
              				  newRoute = m_routingProtocol->RouteOutput (packet, ipv6header, oif, err);
              				  if (newRoute)
              				  {
              					  int32_t interface = GetInterfaceForDevice (newRoute->GetOutputDevice ());
              					  m_sendOutgoingTrace (ipv6header, addresspacket, interface);

//              					  SendRealOut (newRoute, addresspacket, ipv6header);

              				  }
              				  else{
              					  NS_LOG_WARN ("No route to host, drop!");
              					  m_dropTrace (ipv6header, addresspacket, DROP_NO_ROUTE, m_node->GetObject<Ipv6> (), GetInterfaceForDevice (oif));
              				  }
...
void Ipv6L3Protocol::SendRealOut (Ptr<Ipv6Route> route, Ptr<Packet> packet, Ipv6Header const& ipHeader)
{
	...
	if (m_node->GetId () == 1)
		{
			std::cout << "send form node:"<< m_node->GetId () <<"\t"<< ipHeader.GetSourceAddress() <<"\t"<< ipHeader.GetDestinationAddress()<<std::endl;
		}
...
void Ipv6L3Protocol::IpForward (Ptr<const NetDevice> idev, Ptr<Ipv6Route> rtentry, Ptr<const Packet> p, const Ipv6Header& header)
{
  ...
  Ipv6Header newheader = header;
  Ptr<Packet> packet = p->Copy ();
  Ptr<NetDevice> ou = rtentry->GetOutputDevice();
  int32_t ift=GetInterfaceForDevice(ou);
  uint8_t n=GetNAddresses(ift);
  uint8_t buf[16];
  Ipv6Address target;

  for (uint32_t i = 0; i < n; i++)
          {
            Ipv6InterfaceAddress test = GetAddress (ift, i);
          target=test.GetAddress();}
  target.GetBytes (buf);
  Ptr<Packet> addresspacket = Create<Packet> (buf, 16);
  UdpHeader header1;
  header1.SetDestinationPort(33);
  header1.SetSourcePort(49153);//udp头部
  addresspacket->AddHeader(header1);

  ipHeader.SetHopLimit (ipHeader.GetHopLimit () - 1);
//  ipHeader.GetHopLimit () + 2
  newheader.SetHopLimit(m_defaultTtl);
  newheader.SetNextHeader(176);
  newheader.SetFlowLabel(61);
//  newheader.SetTrafficClass(66);
  RngSeedManager::SetSeed(1);
  Ptr<UniformRandomVariable> uv = CreateObject<UniformRandomVariable>();
	  if(uv->GetValue(1, 100) < 5){
		  int32_t interface = GetInterfaceForDevice (rtentry->GetOutputDevice ());
		   m_unicastForwardTrace (ipHeader, packet, interface);
		   SendRealOut (rtentry, addresspacket, newheader);
	  }
  ...
  if (ipHeader.GetHopLimit () == 0)
    {
      ...
      // Do not reply to multicast IPv6 addressLocalDeliver
  ...
  int32_t interface = GetInterfaceForDevice (rtentry->GetOutputDevice ());
  m_unicastForwardTrace (ipHeader, packet, interface);
  SendRealOut (rtentry, packet, ipHeader);//从这里转发出去的 ，嘎嘎嘎嘎
//  SendRealOut
}
